...
const Header = props => {
    ...
    const main_store = useContext(mainContext);
    const auth_store = useContext(authContext);

    useEffect(()=> {
        const cookie = ("; "+document.cookie).split("; authenticated=").pop().split(";").shift();
        if (cookie) {
            axios.post('/api/auth/get-user/', {})
                .then(response=> {
                    console.log(response.data)
                    auth_store.setUser({type:"SET_USER", ...response.data})
                    main_store.dispatchAppState({type: "SET_AUTH", is_authenticated: true})
            })
        }
    }, [])
    ...
    return (
        <header
            className={`navbar navbar-expand-lg align-items-center w-100 position-absolute z-index-10 navbar-${props.bg ? props.bg : 'light'} bg-${props.bg ? props.bg : ""}`}>
            <div className={"position-relative container-fluid"}>
                <Link to={'/'} className={"skip-to-main sr-only-focusable"} role={'button'}>
                    Skip to main content</Link>
                <Link className={"navbar-brand mr-auto"} to={'/'} style={{color: props.logo}}
                      aria-label={'Click to go back to Homepage'}>KaliBlog</Link>
                ...
                {props.nav ? (
                    <div className={"collapse navbar-collapse"} id={'navbarCollapse'}>
                        <ul className={"navbar-nav ml-auto"}>
                            ...
                            <li className={"nav-item dropdown"}>
                                <a className={`nav-link ${props.active === 1 ? "active" : ""} dropdown-toggle`}
                                   href={'#'} id={'navbarPages'} role={'button'} data-toggle={'dropdown'}
                                   aria-haspopup={'true'} aria-expanded={'false'}>
                                    Auth
                                </a>
                            </li>
                            <li className={"nav-item dropdown"}>
                                <a className={`nav-link ${props.active === 1 ? "active" : ""} `} href={'#'}
                                   id={'navbarDocs'} role={'button'} data-toggle={'dropdown'} aria-haspopup={'true'}
                                   aria-expanded={'false'}>
                                    About</a>
                            </li>
                            {
